...
describe Permission do
  ...
  context 'User as guest' do
    subject { Permission.new(user: nil) }
  ...
  context 'User as leader' do
    ...
    subject { Permission.new(user: create(:leader)) }
    ...
    #
    # it { should allow("users", "new") } # These 2: Ideally, iff leader
    # it { should allow("users", "create") }
    # it { should_not allow("users", "edit") } # These 3: Ideally, Should allow iff editing self
    # it { should_not allow("users", "update") }
    # it { should_not allow("users", "destroy") }
  ...
  context 'User as super' do
    ...
    subject { Permission.new(user: create(:super)) }
